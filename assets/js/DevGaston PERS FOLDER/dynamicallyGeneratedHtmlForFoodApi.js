	...
	foodSection.append(foodDisplay);
		...
		foodDisplay.append(foodSearchArea);
			...
			<div class="dropdown">
				...
				let foodSearchButton2 = $(`<button class="btn btn-primary" id="foodSearchButton2">Search</button>`);
		...
		foodDisplay.append(foodDisplayArea);
			...
			let foodResultButton1 = $(`<button class="btn btn-primary" id="foodResultButton1">${foodName[0]}</button>`); //change to API access logic
			let foodPath1 = foodData.name[0]; //change to API access logic
			foodDisplayArea.append(foodResultButton1);

			let foodResultButton2 = $(`<button class="btn btn-primary" id="foodResultButton2">${foodName[1]}</button>`); //change to API access logic
			let foodPath2 = foodData.name[1]; //change to API access logic
			foodDisplayArea.append(foodResultButton2);

			let foodResultButton3 = $(`<button class="btn btn-primary" id="foodResultButton3">${foodName[2]}</button>`); //change to API access logic
			let foodPath3 = foodData.name[2]; //change to API access logic
			foodDisplayArea.append(foodResultButton3);

			let foodResultButton4 = $(`<button class="btn btn-primary" id="foodResultButton4">${foodName[3]}</button>`); //change to API access logic
			let foodPath4 = foodData.name[3]; //change to API access logic
			foodDisplayArea.append(foodResultButton4);

			let foodResultButton5 = $(`<button class="btn btn-primary" id="foodResultButton5">${foodName[4]}</button>`); //change to API access logic
		...
		foodDisplay.append(videoArea);
			...
			let videoSource = foodData.video // [on click event take id and apply to this logic] //change to API access logic
			let videoResultArea = $(`
